...
"""
    ...
    async def _process_command(
        ...
        if intent == "email_collection":
            ...
            return await self._handle_list_all_products(text, user_info, params)
        
        elif intent == "filtered_email_collection":
            return await self._handle_filtered_email_collection(text, user_info, params)
...
say "generate promotional email for [topic]" to continue"""
    ...
    async def _handle_collection_and_campaign(
        ...
        try:
            # 1. Capture manual filters directly extracted by the OpenAI router
            manual_filters = {
                "product_tags": params.get("product_tags", []),
                "product_type": params.get("product_type", ""),
                "total_amount": params.get("total_amount") if params.get("total_amount", 0.0) > 0 else None,
                "status": params.get("status", "active")
            }

            # Check and parse date ranges using your utility helper function
            if params.get("date_range"):
                date_range = parse_date_range_from_text(params["date_range"])
                if date_range:
                    manual_filters["date_range"] = date_range
            else:
                date_range = parse_date_range_from_text(text)
                if date_range:
                    manual_filters["date_range"] = date_range

            # Determine if the user provided ANY manual filtering constraints
            has_manual_filters = bool(
                manual_filters.get("date_range") or 
                manual_filters.get("total_amount") or 
                manual_filters.get("product_type") or 
                manual_filters.get("product_tags")
            )

            # Initialize database lookup filters
            db_filters = {}
            if has_manual_filters:
                # Rule: Use ONLY the manual parameters for audience segment identification
                db_filters = {k: v for k, v in manual_filters.items() if v}
            else:
                # Default status if nothing else is provided
                db_filters["status"] = manual_filters.get("status", "active")

            # 2. Dynamic Shopify Product Resolution Hook
            product_name = params.get("product_name")
            resolved_product_title = "Manual Filters Only"
            content_generation_context = {}

            if product_name and product_name.strip():
                # Always resolve the product from Shopify to gather context for the copywriter
                ...
                # Use the complete, rich raw data from Shopify for content generation (includes description)
                content_generation_context = raw_product_data

                # CRITICAL CONDITION: Only bind Shopify attributes to DB lookup filters if NO manual overrides exist
                if not has_manual_filters:
                    shopify_tags = raw_product_data.get("tags", "")
                    shopify_type = raw_product_data.get("product_type", "")

                    if shopify_type:
                        db_filters["product_type"] = shopify_type
                    if shopify_tags:
                        parsed_tags = [t.strip() for t in shopify_tags.split(",") if t.strip()]
                        db_filters["product_tags"] = parsed_tags
            else:
                # Fallback context if no product name was passed
                content_generation_context = db_filters

            # Clean out empty values one final time before hitting MongoDB
            db_filters = {k: v for k, v in db_filters.items() if v}

           # 3. Execute cross-collection MongoDB lookup pipeline
            matched_profiles = await db_service.get_customers(db_filters)
            
            if not matched_profiles:
                return "I found 0 customers in your transaction database matching those specific criteria to target."

            # 4. Extract real names directly from database properties
            recipients = []
            for p in matched_profiles:
                if p.get("email"):
                    # Use the actual name value if it exists and is valid, otherwise use empty string
                    db_name = p.get("name", "").strip()
                    if db_name and "@" not in db_name:
                        recipient_name = db_name
                    else:
                        recipient_name = "" # Triggers the generic marketing placeholder loop later

                    recipients.append(
                        EmailRecipient(email=p["email"], name=recipient_name)
                    )

            # 5. Serialize the context profile and names are now bound to the list
            topic = params.get("topic", text)
            
            # Pass a subset list of sample recipient names to the copywriter context if needed
            sample_names = [r.name for r in recipients if r.name][:5]
            generation_context = {
                "product_context": content_generation_context,
                "target_sample_names": sample_names
            }
            
            full_context_json = json.dumps(generation_context, indent=2)

            campaign_content = await outreach_tool.generate_campaign_content(
                campaign_type="promotional",
                topic=topic,
                prod_details=full_context_json
            )

            # 6. Extract scheduling properties and save campaign
            scheduled_at = None
            is_immediate = any(word in text.lower() for word in ["now", "immediately", "right away"])
            ...
            campaign_id = await outreach_tool.create_campaign(
                title=f"smart_campaign_{datetime.now().strftime('%Y%m%d_%H%M')}",
                recipients=recipients,
                subject=campaign_content["subject"],
                body=campaign_content["body"],
                scheduled_at=scheduled_at,
            ...
            time_info = f"scheduled: {scheduled_at.strftime('%Y-%m-%d %H:%M')}" if scheduled_at else "sent: immediately"

            return f"""campaign created via smart targeting ✓
campaign id: {campaign_id}
target reference: {resolved_product_title}
matched recipients count: {len(recipients)}
status: {status}
{time_info}

subject: {campaign_content["subject"]}

reply "preview" to verify the generated copywriting draft"""

        except Exception as e:
            import traceback
            traceback.print_exc()
            return f"error executing compound collection and campaign process: {str(e)}"
        
    async def _handle_list_all_products(
        self, text: str, user_info: Dict, params: Dict
    ) -> str:
       
        try:
            # 1. Fetch all available products from Shopify
            products = await shopify_service.get_products(limit=250)
            
            if not products:
                return "No products found in your Shopify store inventory."

            # 2. Iterate and format the output containing only name and type
            response_lines = ["*Available Shopify Products Inventory:*\n"]
            
            for index, product in enumerate(products, 1):
                title = product.get("title", "Unknown Name")
                product_type = product.get("product_type", "N/A")
                
                response_lines.append(f"{index}. *Name:* {title} | *Type:* {product_type}")

            # 3. Join and return the clean text output back to the user
            return "\n".join(response_lines)

        except Exception as e:
            return f"Error retrieving product list: {str(e)}"  

    async def _handle_filtered_email_collection(
        self, text: str, user_info: Dict, params: Dict
    ) -> str:
        try:
            # 1. Extract database segment filters from the AI routing parameters
            db_filters = {
                "source": params.get("source", "database"),
                "product_name": params.get("product_name") if params.get("product_name") else None,
                "product_type": params.get("product_type") if params.get("product_type") else None,
                "product_tags": params.get("product_tags") if params.get("product_tags") else [],
                "total_amount": params.get("total_amount") if params.get("total_amount", 0.0) > 0 else None
            }
            # Handle dynamic natural language date range conversions if available
            if params.get("date_range"):
                date_range = parse_date_range_from_text(params["date_range"])
                if date_range:
                    db_filters["date_range"] = date_range
            else:
                date_range = parse_date_range_from_text(text)
                if date_range:
                    db_filters["date_range"] = date_range

            # Remove empty fields so they don't break the MongoDB search query
            db_filters = {k: v for k, v in db_filters.items() if v is not None}

            # 2. Query the transaction database collections
            matched_profiles = await db_service.get_customers(db_filters)
            
            if not matched_profiles:
                return "I found 0 customers in your transaction database matching those specific criteria to target."

            # 3. Cache the matched results into standard EmailRecipient structures for upcoming campaign steps
            recipients = [
                EmailRecipient(email=p["email"], name=p.get("name", p["email"].split("@")[0]))
                for p in matched_profiles if p.get("email")
            ]
            ...
            # 4. Format a clean output list for the user back in Slack
            email_lines = []
            for idx, r in enumerate(recipients, 1):
                email_lines.append(f"{idx}. *{r.name}* (<mailto:{r.email}|{r.email}>)")
            
            joined_emails = "\n".join(email_lines)

            # Format descriptive criteria summaries
            criteria_summary = []
            if db_filters.get("product_type"):
                criteria_summary.append(f"• Category Type: {db_filters['product_type']}")
            if db_filters.get("product_tags"):
                criteria_summary.append(f"• Target Tags: {', '.join(db_filters['product_tags'])}")
            if db_filters.get("total_amount"):
                criteria_summary.append(f"• Order Amount: >= ${db_filters['total_amount']}")
            if db_filters.get("date_range"):
                criteria_summary.append(f"• Frame: Past {params.get('date_range', 'specified days')}")

            return f"""*Filtered Audience Collection Segment Complete ✓*
Found {len(recipients)} matching customer profiles:
{joined_emails}
*Applied filters:*
{chr(10).join(criteria_summary) if criteria_summary else "• All transactions (No constraints)"}

*Next Steps:*
1. Say "generate promotional email for [topic]" to compose the copy.
2. Say "send campaign" to dispatch or schedule it to this collected segment."""
